...
func main() {
	...
	ep := e.Encode(asm.INST(lvm2.InstructionType_MOV, asm.OPCONST(lvm2.REGISTER_SYS32), asm.OPCONST(1)))             // MOV %SYS32, 1
	e.Encode(asm.INST(lvm2.InstructionType_MOV, asm.OPCONST(lvm2.REGISTER_SYS33), asm.OPCONST(hw)))                  // MOV %SYS33, #hw
	e.Encode(asm.INST(lvm2.InstructionType_MOV, asm.OPCONST(lvm2.REGISTER_SYS34), asm.OPCONST(14)))                  // MOV %SYS34, 14
	e.Encode(asm.INST(lvm2.InstructionType_SYSCALL, asm.OPCONST(lvm2.REGISTER_R0), asm.OPCONST(1), asm.OPCONST(0)))  // SYSCALL %R0, 1, 0
